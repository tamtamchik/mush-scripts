...
Main.LeproWatch.renderLogs = (logs, search = false) => {
    ...
    let lastCycle = -1;
    let lastRoom = '';
    const content = $('#leprowatch_content .logs');
        ...
        if (!content.find(`.cdChatPack[data-c="${log.cycle}"]`).length) {
            ...
            cdChatPack = $(`<div class="cdChatPack" data-c="${log.cycle}"></div>`);
            {
                const day_cycle = $(`<div class="day_cycle" data-c="${log.cycle}"></div>`);
                {
                    ...
                    const imgplus = $(`<img class="cdMax moreless" src="/img/icons/ui/more.png" onclick="Main.maximizeChatPack($(this).parents('.cdChatPack'));return false;" style="display: none;">`);
                    imgplus.appendTo(day_cycle);
                    const divcycle = $('<div>').append(`day ${day} Cycle ${cycle}`);
                    divcycle.appendTo(day_cycle);
                }
                day_cycle.appendTo(cdChatPack);

                const cdContent = $('<div>').addClass('cdContent');
                cdContent.appendTo(cdChatPack);
            }
            cdChatPack.appendTo(content);
        } else {
            cdChatPack = content.find(`.cdChatPack[data-c="${log.cycle}"]`);
        }

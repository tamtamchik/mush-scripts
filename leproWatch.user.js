...
Main.LeproWatch.collectLogs = () => {
    ...
    const logs = records.map((el, index) => {
        ...
        var times=Main.LeproWatch.getTimes(el.getElementsByClassName("ago")[0].innerHTML);
        //noinspection JSUnresolvedVariable
        return {
...
Main.LeproWatch.renderLogs = (logs, search = false) => {
    ...
    logs.forEach(log => {
        ...
        const row = $('<div>').addClass(log.classes).append('<span class="ago" title="'+ Main.LeproWatch.formatTime(log.time)+' ±'+log.accuracy+'">'+Main.LeproWatch.setTimes(log.time)+'</span>'+log.html.trim());
        row.appendTo(content);
    });
};
